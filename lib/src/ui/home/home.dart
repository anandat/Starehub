...
import '../../ui/accounts/account/account1.dart';
...
class _HomeState extends State<Home> with TickerProviderStateMixin {
    ...
    return ScopedModelDescendant<AppStateModel>(
        ...
        return Scaffold(
          ...
          AppBar(
            ...
            bottom: model.blocks.settings.enableHomeTabBar ? TabBar(
  ...
  List<Widget> buildLisOfBlocks(BlocksModel snapshot) {
    ...
    for (var i = 0; i < snapshot.blocks.length; i++) {
      ...
      if ((snapshot.blocks[i].blockType == 'product_block' || snapshot.blocks[i].blockType == 'flash_sale_block') &&
          snapshot.blocks[i].style == 'scroll') {
        var filter = new Map<String, dynamic>();
        filter['id'] = snapshot.blocks[i].linkId.toString();

        list.add(ProductScrollList(block: snapshot.blocks[i], viewAllTitle: snapshot.localeText.viewAll, filter: filter));
      }
      ...
      if(snapshot.blocks[i].blockType == 'on_sale_block' && snapshot.blocks[i].products.length > 0) {
        var filter = new Map<String, dynamic>();
        filter['on_sale'] = '1';
        ...
        if(snapshot.blocks[i].style == 'grid')
          ...
          list.add(ProductGridList(
              ...
              block: snapshot.blocks[i], onProductClick: onProductClick));
      else list.add(ProductScrollList(block: snapshot.blocks[i], viewAllTitle: snapshot.localeText.viewAll, filter: filter));

      }

      if(snapshot.blocks[i].blockType == 'featured_block' && snapshot.blocks[i].products.length > 0) {
        var filter = new Map<String, dynamic>();
        filter['featured'] = '1';
        if(snapshot.blocks[i].style == 'grid')
      ...
      if(snapshot.blocks[i].blockType == 'best_selling' && snapshot.blocks[i].products.length > 0) {
        var filter = new Map<String, dynamic>();
        filter['featured'] = '1';
        if(snapshot.blocks[i].style == 'grid')
  ...
  Widget productText(){
    return Container(child:Text("Products",style:TextStyle(fontSize:20,fontWeight: FontWeight.bold)),
        ...
        /*Container(
          child: InkWell(
            ...
            child: Icon(
              ...
              color: Colors.deepOrange == Colors.white ? Theme.of(context).hintColor : Theme.of(context).primaryIconTheme.color, //Theme.of(context).primaryIconTheme.color,Theme.of(context).hintColor,
            ),
          ),
        ),*/
        Container(
          child: InkWell(
            onTap: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (BuildContext context) => UserAccount1(),
                    fullscreenDialog: true,
                  ));
            },
            child: Icon(
              MStoreIcons.account_circle_line,
              color: Theme.of(context).primaryColor == Colors.white ? Theme.of(context).hintColor : Theme.of(context).primaryIconTheme.color, //Theme.of(context).primaryIconTheme.color,Theme.of(context).hintColor,
            ),
          ),
        ),
        Expanded(
