...
class AppStateModel extends Model {
  ...
  Future<bool> fetchAllBlocks() async {
    page = 1;
  ...
  updateAllBlocks() async {
    ...
    if (response.statusCode == 200) {
      ...
      notifyListeners();
      getCart();
      if(blocks.settings.home != 'HomeNearBy') {
        prefs.setString('customerLocation', '');
      }
  ...
  Future<bool> applyVendor(data, BuildContext context) async {
    print(data);
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=mstore_flutter-apply-vendor', data);
    if (response.statusCode == 200) {
      user = Customer.fromJson(json.decode(response.body));
      if(user.id != null && user.id > 0) {
        _updatePushData();
  ...
  //Shopping Cart
  Future<bool> addToCart(data, BuildContext context) async {
    final response = await apiProvider.post(
  ...
  updateRangeValue(RangeValues newRange) {
    //UnComment this will give error
  ...
  Future<bool> getCartAfterAddingBalanceToCart() async {
    final response = await apiProvider.post('/wp-admin/admin-ajax.php?action=mstore_flutter-cart', Map());
    notifyListeners();
    if (response.statusCode == 200) {
      shoppingCart = CartModel.fromJson(json.decode(response.body));
      updateCartCount();
  ...
  getDeliverySlot(dateParam) async {
    ...
    if (response.statusCode == 200) {
      ...
      if(deliverySlot[dateParam].slots[0] != null) {
